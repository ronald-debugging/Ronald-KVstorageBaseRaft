...
void MprpcChannel::CallMethod(const google::protobuf::MethodDescriptor* method,
  ...
  if (m_clientFd == -1) {
    ...
    if (!rt) {
      DPrintf("[func-MprpcChannel::CallMethod]Reconnection to ip: {%s} port{%d} failed", m_ip.c_str(), m_port);
      controller->SetFailed(errMsg);
      return;
    } else {
      DPrintf("[func-MprpcChannel::CallMethod]Connection to ip: {%s} port{%d} successful", m_ip.c_str(), m_port);
    }
  }
  ...
  while (-1 == send(m_clientFd, send_rpc_str.c_str(), send_rpc_str.size(), 0)) {
    ...
    std::cout << "Attempting to reconnect, remote IP: " << m_ip << " remote port: " << m_port << std::endl;
    close(m_clientFd);
    m_clientFd = -1;
    std::string errMsg;
    bool rt = newConnect(m_ip.c_str(), m_port, &errMsg);
