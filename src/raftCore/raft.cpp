...
void Raft::AppendEntries1(const raftRpcProctoc::AppendEntriesArgs* args, raftRpcProctoc::AppendEntriesReply* reply) {
  ...
  if (args->term() < m_currentTerm) {
    ...
    DPrintf("[func-AppendEntries-rf{%d}] Rejected because Leader{%d}'s term{%v} < rf{%d}.term{%d}\n", m_me, args->leaderid(),
    ...
    for (int i = 0; i < args->entries_size(); i++) {
      ...
      if (log.logindex() > getLastLogIndex()) {
        ...
        if (m_logs[getSlicesIndexFromLogIndex(log.logindex())].logterm() == log.logterm() &&
          ...
          myAssert(false, format("[func-AppendEntries-rf{%d}] Two nodes have the same logIndex{%d} and term{%d}, but their commands{%d:%d}   "
                                 " {%d:%d} are different!!\n",
...
void Raft::applierTicker() {
  while (true) {
    ...
    if (!applyMsgs.empty()) {
      DPrintf("[func- Raft::applierTicker()-raft{%d}] Reporting applyMsgs length to kvserver: {%d}", m_me, applyMsgs.size());
    }
...
void Raft::doElection() {
  ...
  if (m_status != Leader) {
    DPrintf("[       ticker-func-rf(%d)              ]  Election timer expired and not a leader, starting election \n", m_me);
...
void Raft::doHeartBeat() {
  ...
  if (m_status == Leader) {
    DPrintf("[func-Raft::doHeartBeat()-Leader: {%d}] Leader's heartbeat timer triggered and got mutex, starting to send AE\n", m_me);
    ...
    for (int i = 0; i < m_peers.size(); i++) {
      ...
      DPrintf("[func-Raft::doHeartBeat()-Leader: {%d}] Leader's heartbeat timer triggered index:{%d}\n", m_me, i);
...
void Raft::electionTimeOutTicker() {
  ...
  while (true) {
    ...
    if (std::chrono::duration<double, std::milli>(suitableSleepTime).count() > 1) {
      ...
      std::cout << "\033[1;35m electionTimeOutTicker(); function set sleep time to: "
                << std::chrono::duration_cast<std::chrono::milliseconds>(suitableSleepTime).count() << " ms\033[0m"
                << std::endl;
      std::cout << "\033[1;35m electionTimeOutTicker(); function actual sleep time: " << duration.count() << " ms\033[0m"
                << std::endl;
    }
...
void Raft::leaderHearBeatTicker() {
  while (true) {
    ...
    if (std::chrono::duration<double, std::milli>(suitableSleepTime).count() > 1) {
      std::cout << atomicCount << "\033[1;35m leaderHearBeatTicker(); function set sleep time to: "
                << std::chrono::duration_cast<std::chrono::milliseconds>(suitableSleepTime).count() << " ms\033[0m"
                << std::endl;
      // 获取当前时间点
      auto start = std::chrono::steady_clock::now();
      ...
      // 使用ANSI控制序列将输出颜色修改为紫色
      std::cout << atomicCount << "\033[1;35m leaderHearBeatTicker(); function actual sleep time: " << duration.count()
                << " ms\033[0m" << std::endl;
      ++atomicCount;
    }
...
void Raft::RequestVote(const raftRpcProctoc::RequestVoteArgs* args, raftRpcProctoc::RequestVoteReply* reply) {
  ...
  myAssert(args->term() == m_currentTerm,
           format("[func--rf{%d}] Previously verified args.Term==rf.currentTerm, but here they are not equal", m_me));
...
bool Raft::sendRequestVote(int server, std::shared_ptr<raftRpcProctoc::RequestVoteArgs> args,
  ...
  // todo
  auto start = now();
  DPrintf("[func-sendRequestVote rf{%d}] sending RequestVote to server{%d} started", m_me, m_currentTerm, getLastLogIndex());
  bool ok = m_peers[server]->RequestVote(args.get(), reply.get());
  DPrintf("[func-sendRequestVote rf{%d}] sending RequestVote to server{%d} completed, time taken:{%d} ms", m_me, m_currentTerm,
  ...
  if (*votedNum >= m_peers.size() / 2 + 1) {
    ...
    *votedNum = 0;
    if (m_status == Leader) {
      ...
      myAssert(false,
               format("[func-sendRequestVote-rf{%d}]  term:{%d} same term elected as leader twice, error", m_me, m_currentTerm));
    }
...
bool Raft::sendAppendEntries(int server, std::shared_ptr<raftRpcProctoc::AppendEntriesArgs> args,
  ...
  DPrintf("[func-Raft::sendAppendEntries-raft{%d}] leader sending AE rpc to node {%d} started, args->entries_size():{%d}", m_me,
          server, args->entries_size());
  bool ok = m_peers[server]->AppendEntries(args.get(), reply.get());

  if (!ok) {
    DPrintf("[func-Raft::sendAppendEntries-raft{%d}] leader sending AE rpc to node {%d} failed", m_me, server);
    return ok;
  }
  DPrintf("[func-Raft::sendAppendEntries-raft{%d}] leader sending AE rpc to node {%d} successful", m_me, server);
  ...
  if (!reply->success()) {
    ...
    if (reply->updatenextindex() != -100) {
      ...
      DPrintf("[func -sendAppendEntries  rf{%d}]  Returned log term is equal, but doesn't match, shrinking nextIndex[%d]: {%d}\n", m_me,
    ...
    *appendNums = *appendNums + 1;
    DPrintf("---------------------------tmp------------------------- Node{%d} returned true, current *appendNums{%d}", server,
            *appendNums);
    ...
    if (*appendNums >= 1 + m_peers.size() / 2) {
      ...
      *appendNums = 0;
      ...
      if (args->entries_size() > 0 && args->entries(args->entries_size() - 1).logterm() == m_currentTerm) {
        DPrintf(
            "---------------------------tmp------------------------- Current term has log successfully committed, updating leader's m_commitIndex "
            "from{%d} to{%d}",
