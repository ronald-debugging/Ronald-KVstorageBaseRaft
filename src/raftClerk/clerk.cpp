...
void Clerk::PutAppend(std::string key, std::string value, std::string op) {
  ...
  while (true) {
    ...
    if (!ok || reply.err() == ErrWrongLeader) {
      DPrintf("【Clerk::PutAppend】Previous assumed leader: {%d} request failed, retrying with new leader {%d}, operation: {%s}", server, server + 1,
              op.c_str());
      if (!ok) {
        DPrintf("Retry reason: RPC failure");
      }
      if (reply.err() == ErrWrongLeader) {
        DPrintf("Retry reason: Not a leader");
      }
      server = (server + 1) % m_servers.size();  // try the next server
      continue;
    }
    if (reply.err() == OK) {  //When will reply errno be ok???
      m_recentLeaderId = server;
      return;
    }
  }
